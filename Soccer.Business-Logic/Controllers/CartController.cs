...
//        [HttpPost("update")]
//        public async Task<IActionResult> UpdateCartItem(int productId, int sizeId, int quantity)
//        {
...
namespace Soccer.Business_Logic.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        [HttpPost("update")]
        public async Task<IActionResult> UpdateCartItem(int productId, int sizeId, int quantity, [FromBody] string description = null)
        {
            if (quantity < 1) return BadRequest("Quantity must be at least 1");
            ...
            cartItem.UnitPrice = product.SalePrice * quantity;
            cartItem.UnitCost = product.CostPrice * quantity;
            cartItem.Description = description; // Cập nhật mô tả, null nếu không có
